...
func TestParseSemver(t *testing.T) {
	tests := []struct {
		...
		{"1.2.3", 1, 2, 3, true},
		{"v0.0.1", 0, 0, 0, false}, // v not allowed
		{"3.2.3", 3, 2, 3, true},
...
func TestMatchVersion(t *testing.T) {
	versions := []string{
		"v0.0.1", "0.1.3", "1.1.1", "v1.2.3", "1.2.4", "v1.3.0", "v2.0.0",
		"2.1.1", "v2.2.0", "v3.0.0", "v3.0.1", "v3.1.4", "v3.1.5",
		"v4.0.0", "4.0.4", "v4.3.1", "5.0.0", "7.2.1", "v9.0.0",
	...
	tests := []struct {
		...
		{"^2.1.2", "v2.2.0", "caret match within major 2"},
		...
		{">=3.2.0", "v10.3.1", "upper range found"},
		{"<=0.2.0", "0.1.3", "lower range  found"},
	}

	for _, tt := range tests {
		result := GetLatestVersionByConstraint(versions, tt.target)
